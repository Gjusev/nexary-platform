...
import { presignedPutObject } from '@/lib/storage/minio';
...
export async function POST(
  ...
  try {
    ...
    const objectKey = `team/${pkg.teamSlug}/packages/${packageId}/docs/${Date.now()}-${filename}`;
    try {
      const url = await presignedPutObject(bucket, objectKey, 900);
      return NextResponse.json({ bucket, objectKey, url, expiresIn: 900, contentType: contentType || 'application/octet-stream' });
    } catch (e) {
      return NextResponse.json({ error: 'MinIO not configured' }, { status: 501 });
    }
  } catch (e) {
